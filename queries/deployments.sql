  ...
  (
    (# Cloud Build, Github, Gitlab pipelines
      ...
      CASE WHEN source = "cloud_build" then JSON_EXTRACT_SCALAR(metadata, '$.substitutions.COMMIT_SHA')
           WHEN source like "drone%" then JSON_EXTRACT_SCALAR(metadata, '$.deployment_sha')
      ...
      OR (source LIKE "drone%" and event_type = "deployment" and JSON_EXTRACT_SCALAR(metadata, '$.deployment_status') = "SUCCESS"))
    )
    UNION ALL
    (# Tekton Pipelines
      SELECT
